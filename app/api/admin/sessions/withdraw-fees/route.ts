...
import { recordChainWithdrawFees } from "../../../../lib/server/spotr-store";
import { verifySignedSpotrAction } from "../../../../lib/server/signed-action";
import {
  ValidationError,
  parseAdminWithdrawFeesPayload,
  parseSignedEnvelope,
...
export async function POST(request: Request) {
  try {
    ...
    const envelope = parseSignedEnvelope(body, parseAdminWithdrawFeesPayload);
    consumeSignedActionToken(
      envelope.walletAddress,
      "admin/sessions/withdraw-fees"
    );
    const { payload } = await verifySignedSpotrAction(
      "admin-withdraw-protocol-fees",
      envelope
    );
    await recordChainWithdrawFees({
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to record withdraw_protocol_fees.",
      },
      { status }
    );
  }
