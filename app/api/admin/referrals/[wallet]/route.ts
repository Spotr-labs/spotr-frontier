...
import { getAdminReferrerDetail } from "../../../../lib/server/spotr-store";
...
export async function GET(
  ...
  try {
    ...
    const payload = await getAdminReferrerDetail({
      walletAddress: adminWallet,
      referrerWallet: target,
    });
    if (!payload) {
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Failed to load referrer.",
      },
      { status }
    );
  }
